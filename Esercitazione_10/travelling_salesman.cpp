...
void Travelling_salesman::ChangeBest(int *labels){
    
    sort(m_population.begin(), m_population.end(), FitCompare);
    
    vector <City> new_path;
    for(int i=0; i<m_cities.size(); i++){
        
        for(int j=0; j<m_cities.size(); j++){
            if(labels[i]==m_cities[j].GetLabel()) new_path.push_back(m_cities[j]);
        }
    }
    
    m_population[0].SetPath(new_path);

};

void Travelling_salesman::ChangeWorst(int *labels){
    ...
    m_population[m_population.size()-1].SetPath(new_path);
