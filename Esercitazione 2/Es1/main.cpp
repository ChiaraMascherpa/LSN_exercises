...
double my_distr(double x){
    return 1-sqrt(1-x);                            // Inverse of the cumulative used to get points distributed as the distribution
}

double my_val (double x){
    return my_func(x)/2/(1-x);                     // Dividing the integral by the distribution
}

double my_distr2(double x){
    return sqrt(x);                                // Inverse of the cumulative used to get points distributed as the distribution

double my_val2 (double x){
    return my_func(x)/2./x;                        // Dividing the integral by the distribution
}
...
int main(){
    ...
    Integral integral3 (0,1, my_val2);              // To integrate using importance sampling
    ...
    double avec=0;                                 // Saving all sum with importance sampling
    double av2c=0;
    ...
    for(int i=0; i<N; i++){
        ...
        double c=integral3.Distr(L, my_distr2);
        ...
        if(i==0){
            ...
            output<<sqrt((av2b/(i+1) - pow(aveb/(i+1),2))/(i))<<" ";
            output<<sqrt((av2c/(i+1) - pow(avec/(i+1),2))/(i))<<endl;
        }
    }
